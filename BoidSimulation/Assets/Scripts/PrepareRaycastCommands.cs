...
public struct PrepareRaycastCommands : IJobParallelFor
{
    ...
    /// <summary>Write-only array for storing prepared raycast commands. Desired number of commands are created for each
    /// Boid and stored in the array next to each other.</summary>
    ...
    public void Execute(int index)
    {
        ...
        var boidForward = boid.Velocity.normalized; // Boids are always orientated in the direction they are going
        ...
        RaycastCommands[index] =
            new RaycastCommand(SimulationOrigin + boid.Position, raycastDirection, queryParameters, RaycastDistance);
    }
}